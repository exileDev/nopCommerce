...
namespace Nop.Tests
{
    ...
    public partial class SqLiteNopDataProvider : INopDataProvider
    {
        ...
        public async void CreateDatabase(string collation, int triesToConnect = 10)
        {
        ...
        public virtual async Task<IDbConnection> CreateDbConnectionAsync(string connectionString = null)
        {
            _connection ??= new SqliteConnection(string.IsNullOrEmpty(connectionString)
                ? (await DataSettingsManager.LoadSettingsAsync()).ConnectionString
                : connectionString);
        ...
        {
            return await Task.FromResult(GetTable<TEntity>());
        }
        ...
        public async Task<int?> GetTableIdentAsync<TEntity>() where TEntity : BaseEntity
        {
            using (new ReaderWriteLockDisposable(_locker, ReaderWriteLockType.Read))
            {
                ...
                var result = await DataContext.QueryToListAsync<int?>($"select seq from sqlite_sequence where name = \"{tableName}\"");
        ...
        {
            using (new ReaderWriteLockDisposable(_locker))
                await DataContext.ExecuteAsync("VACUUM;");
        }
        ...
        public async Task SetTableIdentAsync<TEntity>(int ident) where TEntity : BaseEntity
        {
            using (new ReaderWriteLockDisposable(_locker))
            {
                ...
                await DataContext.ExecuteAsync($"update sqlite_sequence set seq = {ident} where name = \"{tableName}\"");
            }
        }
        ...
        public async Task<ITempDataStorage<TItem>> CreateTempDataStorageAsync<TItem>(string storageKey, IQueryable<TItem> query) where TItem : class
        {
        ...
        protected DataConnection DataContext => _dataContext ??= new DataConnection(LinqToDbDataProvider, CreateDbConnectionAsync().Result, AdditionalSchema)
        {
            CommandTimeout = DataSettingsManager.GetSqlCommandTimeoutAsync().Result
        };
